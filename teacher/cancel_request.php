...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['request_id']) && isset($_POST['cancel_request'])) {
    ...
    $teacherId = $_SESSION['user_id']; // Get the current teacher's ID instead of student's

    // Check if the request belongs to the current teacher
    $checkSql = "SELECT RequestID FROM room_requests WHERE RequestID = ? AND TeacherID = ? AND Status = 'pending'";
    $checkStmt = $conn->prepare($checkSql);
    $checkStmt->bind_param("ii", $requestId, $teacherId);
    $checkStmt->execute();
    $checkResult = $checkStmt->get_result();

    if ($checkResult->num_rows > 0) {
        // The request exists and belongs to the current teacher, proceed with deletion

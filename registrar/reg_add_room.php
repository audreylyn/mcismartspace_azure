...
<?php include "includes/add_room.php"; ?>
                ...
                <div class="card">
                    <header class="card-header">
                        <div class="new-title-container">
                            <p class="new-title">Room List</p>
                        </div>
                    </header>
                    <div class="card-content">
                        <table id="roomTable" class="table is-fullwidth is-striped">
                            <thead>
                                <tr class="titles">
                                    <th>Room Name</th>
                                    <th>Room Type</th>
                            ...
                            <tbody>
                                <?php if (empty($requests)): ?>
                                    <tr>
                                        <td colspan="5" class="has-text-centered">No rooms found.</td>
                                    </tr>
                                <?php else: ?>
                                    <?php foreach ($requests as $row): ?>
                                        <tr>
                                            <td data-label="Room Name"><?= htmlspecialchars($row['room_name']) ?></td>
                                            <td data-label="Room Type"><?= htmlspecialchars($row['room_type']) ?></td>
                                            <td data-label="Capacity"><?= htmlspecialchars($row['capacity']) ?></td>
                                            <td data-label="Building"><?= htmlspecialchars($row['building_name']) ?></td>
                        ...
                        <form method="POST" class="form">
                            <div class="field is-grouped">
                                <div class="control">
                                    ...
                                    <input class="input" type="text" name="room_name" required>
                                </div>
                                <div class="control">
                                ...
                                <div class="control">
                                    ...
                                    <div class="select">
                                        <select name="building_id" required>
                                            ...
                                            try {
                                                $conn = db();
                                                $stmt = $conn->prepare("SELECT id, building_name FROM buildings");
                                                $stmt->execute();
                                                $buildings = $stmt->get_result();
                                                while ($building = $buildings->fetch_assoc()) {
                                                    echo '<option value="' . htmlspecialchars($building['id']) . '">' .
                            ...
                            <div class="field">
                                <div class="control">
                                    <button type="submit" name="add_room" class="styled-button">Add Room</button>
                                </div>
                            </div>
                        </form>
                    </div>
